...
#ifndef USE_HMM
#define bwtree_value_t value_t
#else
#define bwtree_value_t uint64_t
...
#define MAPPING_TABLE_SIZE ((size_t)(1 << 25))
...
//    printf("thread_id:%d, thread_num:%d\n", thread_id, thread_num);
...
class BwTree : public BwTreeBase {
 ...
 public:
   ...
   */
public:void UpdateThreadLocal(size_t p_thread_num) {
  ...
  inline const BaseNode *GetNode(const NodeID node_id) {
    ...
    assert(node_id != INVALID_NODE_ID);
      if(node_id > (MAPPING_TABLE_SIZE - 10)){
          printf("node_id:%ld, table_size:%ld\n", node_id, MAPPING_TABLE_SIZE);
      }
    assert(node_id < MAPPING_TABLE_SIZE);
