...
int g_order = DEFAULT_BPTREE_ORDER;
b_node *queue;
...
{	
	...
	root = static_cast<b_root_obj *>(malloc(sizeof(*root)));
	root->b_root = NULL;
	printf("b+tree of order %d\n", DEFAULT_BPTREE_ORDER);
	return root;
}

value_t *search_record(b_node *leaf, uint64_t target_key, bool flag)
{
#ifndef B_SEARCH	
	int i;
	static int count;
...
value_t *bp_tree_read(void *p_root, uint64_t key)
{	
	b_root_obj *_root;
	b_node *root, *leaf;
	value_t *val;
		...
		int range, uint64_t *buff)
{
	...
	while (leaf) {
		...
		if (finish)
			break;
		leaf = static_cast<b_node *>(leaf->ptrs[g_order - 1]);
		i = 0;
	}
	return count;
}

int bp_tree_scan(void *p_root, uint64_t start_key, int range,
		uint64_t *buff)
{
	b_root_obj *_root;
	b_node *root;
...
b_node *create_node()
{
	...
	new_node = static_cast<b_node *>(malloc(sizeof(*new_node)));
	if (!new_node) {
...
b_node *create_bp_tree(uint64_t key, bp_val_t rec)
{
	b_node *root;
		...
		int left_index, uint64_t key, b_node *right)
{
	...
	for (i = 0, j = 0; i < old_node->num_keys + 1; ++i, ++j) {
		if (j == left_index + 1)
			++j;
		temp_ptrs[j] = static_cast<b_node *>(old_node->ptrs[i]);
	}
	...
	for (i = 0; i <= new_node->num_keys; ++i) {
		child = static_cast<b_node *>(new_node->ptrs[i]);
                               ...
                               uint64_t key, bp_val_t rec)
{
    ...
    *(bp_val_t*)v = rec;
	...
	temp_keys = static_cast<uint64_t *>(malloc(g_order * sizeof(uint64_t)));
	if (!temp_keys) {
		perror("leaf split failed");
		exit(0);
	}
	temp_ptrs = static_cast<void **>(malloc(g_order * sizeof(void *)));
	if (!temp_ptrs) {
		perror("leaf split failed");
		exit(0);
	}
...
int bp_tree_insert(void *p_root, uint64_t key,
                   bp_val_t val)
{
	b_node *leaf = NULL;
	b_root_obj *_root;
